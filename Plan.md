create a weather app that shows the weather condition use to get the exact weather condition it should also forecast the weather condition in upcomin time it should also tell about humidity wind sunrise sunset aqi and there should be current temp visible acc to the user location so that first he can easily see the temperature or weather in his area the ui of app should be according to the weather outside like in day if sunny there should be a sun and a bright day ui and if raining then a cloud with rain and also for cloudy or windy or at night respectively  the whole ui should be smooth and easily understandable for the user. 
64eaf1ab7c7841ec82f122535261401 api key 
Introduction
...
instruction ; don't run on chrome